...
- saveToBase64Async() - returns base64
- saveToFlatOpcAsync() - returns flatOpc
...
(function (root) {
    ...
    function defineOpenXml(Enumerable, Ltxml, XEnumerable, async) {
        var openXml = {};
        ...
        function openFromZipAsync(zip, pkg, cb) {
            async.forEachOf(zip.files, function (value, f, callback) {
                var zipFile = value;
                if (!openXml.util.endsWith(f, "/")) {
                    ...
                    if (f !== "[Content_Types].xml")
                        f2 = "/" + f;
                    var promise = (openXml.util.endsWith(f, "xml") ? zipFile.async("string") : zipFile.async("binarystring"));
                    promise.then(function (content) {
                        var newPart = new openXml.OpenXmlPart(pkg, f2, null, null, content);
                ...
                for (var part in pkg.parts) {
                    ...
                    thisPart.partType = (openXml.util.endsWith(ct, "xml") ? "xml" : "binary");
                }
                cb(pkg);
            });
        }
        ...
        function openFromBase64InternalAsync(pkg, base64data, cb) {
            ...
            zip.loadAsync(base64data, {
                ...
                openFromZipAsync(new_zip, self, cb);
            });
        }

        function openFromBlobInternalAsync(pkg, blob, cb) {
            var zip = new JSZip();
            zip.loadAsync(blob).then(function (new_zip) {
                openFromZipAsync(new_zip, pkg, cb);
            });
        }
        ...
        openXml.OpenXmlPackage.prototype.openByDataTypeAsync = function (documentToOpen, cb) {
            var self = this;
            if (documentToOpen !== undefined) {
                if (openXml.util.isBase64(documentToOpen)) {
                    openFromBase64InternalAsync(self, documentToOpen, cb);
                }
                else if (typeof documentToOpen === 'string') {
                    openFromFlatOpcInternalAsync(self, documentToOpen, cb);
                }
                else {
                    openFromBlobInternalAsync(self, documentToOpen, cb);
                }
            }
        }
        ...
        openXml.OpenXmlPart.prototype.getXDocument = function () {
            if (this.partType === 'xml' && typeof this.data === 'string') {
                var data = this.data;
                //var utf = openXml.util.decode_utf8(escape(data));
                this.data = XDocument.parse(data);
                this.partType = 'xml';
            }
            return this.data;
        };
    ...
    if (typeof define === typeof function () { } && define.amd) { // AMD
        define("openxml", ["linq", "ltxml", "ltxml-extensions"],
            function (Enumerable, Ltxml, XEnumerable, async) {
                var openXml = defineOpenXml(Enumerable, Ltxml, XEnumerable, async);
                return openXml;
            });
    }
